...
SIV3D_DISABLE_MSVC_WARNINGS_PUSH(4312)
...
SDL_wcslcpy(wchar_t *dst, const wchar_t *src, size_t maxlen)
{
	...
	if (maxlen > 0) {
		size_t len = std::min<size_t>(srclen, maxlen - 1);
...
void
WIN_InitKeyboard(SDL_VideoData* data)
{
	...
	data->ime_himc = 0;
	data->ime_composition_attributes.fill(0);
	data->ime_composition[0] = 0;
	data->ime_readingstring[0] = 0;
	data->ime_cursor = 0;

	data->ime_candlist = SDL_FALSE;
	memset(data->ime_candidates, 0, sizeof(data->ime_candidates));
	...
	//::memset(&data->ime_rect, 0, sizeof(data->ime_rect));
	//::memset(&data->ime_candlistrect, 0, sizeof(data->ime_candlistrect));
	data->ime_winwidth = 0;
...
//
//	for (i = 0; i < _countof(windows_scancode_table); i++) {
//		int vk;
...
void
WIN_StartTextInput(SDL_VideoData *videodata, HWND hwnd)
{
//#ifndef SDL_DISABLE_WINDOWS_IME
//	SDL_Window *window;
//#endif
...
void
WIN_StopTextInput(SDL_VideoData *videodata, HWND hwnd)
{
//#ifndef SDL_DISABLE_WINDOWS_IME
//	SDL_Window *window;
//#endif
...
static BOOL UILess_SetupSinks(SDL_VideoData *videodata);
...
IME_Init(SDL_VideoData *videodata, HWND hwnd)
{
	...
	if (/*SUCCEEDED(WIN_CoInitialize())*/ true) {
...
void
IME_Disable(SDL_VideoData *videodata, HWND)
{
	if (!videodata->ime_initialized || !videodata->ime_hwnd_current)
		return;
...
IME_GetReadingString(SDL_VideoData *videodata, HWND hwnd)
{
	...
	//	}
	//	if (s) {
	//		size_t size = std::min((size_t)(len + 1), _countof(videodata->ime_readingstring));
...
IME_GetCompositionString(SDL_VideoData *videodata, HIMC himc, DWORD string)
{
	...
	//LOG_TEST(U"length:{}"_fmt(length));
	
	if (length < 0)
		length = 0;
	//const size_t CN = length / 2;
	//for(size_t i = 0; i < CN; ++i)
	//	LOG_TEST(U"con:`{}`"_fmt((s3d::char32)videodata->ime_composition[i]));

	length /= sizeof(videodata->ime_composition[0]);
	videodata->ime_cursor = LOWORD(ImmGetCompositionStringW(himc, GCS_CURSORPOS, 0, 0));
	//if (videodata->ime_cursor < _countof(videodata->ime_composition) && videodata->ime_composition[videodata->ime_cursor] == 0x3000) {
	//	int i;
	...
	videodata->ime_composition_attributes.fill(0);

	//for (size_t i = 0; i < CN; ++i)
	//	LOG_TEST(U"con:`{}`"_fmt((s3d::char32)videodata->ime_composition[i]));


	ImmGetCompositionStringW(himc, GCS_COMPATTR, videodata->ime_composition_attributes.data(), (DWORD)videodata->ime_composition_attributes.size());
}
...
IME_SendEditingEvent(SDL_VideoData *videodata)
{
	...
	if (videodata->ime_readingstring[0]) {
		size_t len = std::min(::wcslen(videodata->ime_composition), (size_t)videodata->ime_cursor);
		
		buffer.append(videodata->ime_composition, len);
		buffer.append(videodata->ime_readingstring);
		buffer.append(&videodata->ime_composition[len]);
	}
	else {	
		buffer.append(videodata->ime_composition);
	}

	//LOG_TEST(U"RED: `{}`"_fmt(s3d::Unicode::FromWString(videodata->ime_readingstring)));

	//LOG_TEST(U"C: `{}`"_fmt(videodata->ime_cursor));
	//LOG_TEST(U"ATTR: `{}`"_fmt(videodata->ime_composition_attributes));

	int targetLength = 0;

	for (int i = videodata->ime_cursor; i < SDL_TEXTEDITINGEVENT_TEXT_SIZE; ++i)
	{
		if (videodata->ime_composition_attributes[i] == ATTR_TARGET_CONVERTED)
		{
			++targetLength;
		}
		else
	...
	videodata->pTextInput->sendEditingText(s3d::Unicode::FromWString(buffer), videodata->ime_cursor, targetLength);
	//SDL_SendEditingText(s, videodata->ime_cursor + (int)SDL_wcslen(videodata->ime_readingstring), 0);

	s3d::Array<s3d::String> cands;

	for (int i = 0; i < std::min<int>(videodata->ime_candcount, MAX_CANDLIST); ++i)
	{
		cands << s3d::Unicode::FromWString(videodata->ime_candidates[i]);
	}

	videodata->pTextInput->sendCandidates(cands);
}

static void
IME_AddCandidate(SDL_VideoData *videodata, UINT i, LPCWSTR candidate)
{
	//LOG_TEST(U"CAN {}: `{}`"_fmt(i, s3d::Unicode::FromWString(candidate)));
...
IME_GetCandidateList(HIMC himc, SDL_VideoData *videodata)
{
	...
	if (size) {
		cand_list = (LPCANDIDATELIST)::malloc(size);
		if (cand_list) {
			...
			if (size) {
				...
				if (LANG() == LANG_CHS && IME_GetId(videodata, 0)) {
					...
					for (i = 0; i < videodata->ime_candcount; ++i) {
						size_t len = ::wcslen((LPWSTR)((DWORD_PTR)cand_list + cand_list->dwOffset[i])) + 1;
					...
					videodata->ime_candpgsize = std::min<DWORD>(cand_list->dwPageSize, MAX_CANDLIST);
...
IME_HandleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM *lParam, SDL_VideoData *videodata)
{
	...
	case WM_IME_COMPOSITION:
		//LOG_TEST(U"Composition");
		trap = SDL_TRUE;
		himc = ImmGetContext(hwnd);
		if (*lParam & GCS_RESULTSTR) {
			//LOG_TEST(U"GCS_RESULTSTR");
			IME_GetCompositionString(videodata, himc, GCS_RESULTSTR);
			IME_SendInputEvent(videodata);
		}
		if (*lParam & GCS_COMPSTR) {
			//LOG_TEST(U"GCS_COMPSTR");
			if (!videodata->ime_uiless)
				videodata->ime_readingstring[0] = 0;
...
UILess_GetCandidateList(SDL_VideoData *videodata, ITfCandidateListUIElement *pcandlist)
{
	...
	if (pgcount > 0) {
		UINT *idxlist = (UINT*)::malloc(sizeof(UINT) * pgcount);
		if (idxlist) {
			pcandlist->GetPageIndex(idxlist, pgcount, &pgcount);
			pgstart = idxlist[page];
			if (page < pgcount - 1)
				pgsize = std::min(count, idxlist[page + 1]) - pgstart;
			else
	...
	videodata->ime_candpgsize = std::min<DWORD>(pgsize, MAX_CANDLIST);
...
STDMETHODIMP UIElementSink_UpdateUIElement(TSFSink *sink, DWORD dwUIElementId)
{
	...
	if (SUCCEEDED(element->QueryInterface(IID_ITfReadingInformationUIElement, (LPVOID *)&preading))) {
		...
		if (SUCCEEDED(preading->GetString(&bstr)) && bstr) {
			WCHAR *s = (WCHAR *)bstr;
			SDL_wcslcpy(videodata->ime_readingstring, s, _countof(videodata->ime_readingstring));
			IME_SendEditingEvent(videodata);
			SysFreeString(bstr);
		}
		preading->Release();
...
STDMETHODIMP IPPASink_OnActivated(TSFSink *sink, DWORD, LANGID, REFCLSID, REFGUID catid, REFGUID guidProfile, HKL, DWORD dwFlags)
{
	static const GUID cTF_PROFILE_DAYI = { 0x037B2C25, 0x480C, 0x4D7F,{ 0xB0, 0x27, 0xD6, 0xCA, 0x6B, 0x69, 0x78, 0x8A } };
	SDL_VideoData *videodata = (SDL_VideoData *)sink->data;
	videodata->ime_candlistindexbase = (cTF_PROFILE_DAYI == guidProfile) ? 0 : 1;
	if ((catid == GUID_TFCAT_TIP_KEYBOARD) && (dwFlags & TF_IPSINK_FLAG_ACTIVE))
...
UILess_EnableUIUpdates(SDL_VideoData *videodata)
{
	...
	if (SUCCEEDED(videodata->ime_threadmgrex->QueryInterface(IID_ITfSource, (LPVOID *)&source))) {
		source->AdviseSink(IID_ITfUIElementSink, (IUnknown *)videodata->ime_uielemsink, &videodata->ime_uielemsinkcookie);
		source->Release();
	}
}
...
UILess_DisableUIUpdates(SDL_VideoData *videodata)
{
	...
	if (SUCCEEDED(videodata->ime_threadmgrex->QueryInterface(IID_ITfSource, (LPVOID *)&source))) {
		source->UnadviseSink(videodata->ime_uielemsinkcookie);
...
UILess_SetupSinks(SDL_VideoData *videodata)
{
	...
	if (FAILED(videodata->ime_threadmgrex->ActivateEx(&clientid, TF_TMAE_UIELEMENTENABLEDONLY)))
		return SDL_FALSE;

	videodata->ime_uielemsink = (TSFSink*)::malloc(sizeof(TSFSink));
	videodata->ime_ippasink = (TSFSink*)::malloc(sizeof(TSFSink));
	...
	if (SUCCEEDED(videodata->ime_threadmgrex->QueryInterface(IID_ITfSource, (LPVOID *)&source))) {
		if (SUCCEEDED(source->AdviseSink(IID_ITfUIElementSink, (IUnknown *)videodata->ime_uielemsink, &videodata->ime_uielemsinkcookie))) {
			if (SUCCEEDED(source->AdviseSink(IID_ITfInputProcessorProfileActivationSink, (IUnknown *)videodata->ime_ippasink, &videodata->ime_alpnsinkcookie))) {
				result = SDL_TRUE;
			}
		}
		source->Release();
...
{
	ITfSource *source = 0;
	if (videodata->ime_threadmgrex && SUCCEEDED(videodata->ime_threadmgrex->QueryInterface(IID_ITfSource, (LPVOID *)&source))) {
		source->UnadviseSink(videodata->ime_uielemsinkcookie);
		source->UnadviseSink(videodata->ime_alpnsinkcookie);
		SAFE_RELEASE(source);
		videodata->ime_threadmgrex->Deactivate();
...
static void
IME_DestroyTextures(SDL_VideoData*)
{
}
...
//	BOOL vertical = videodata->ime_candvertical;
//
